...
class DoubleTest {
    ...
    private static void initializeExpectedResults() {
        ...
        EXPECTED_RESULTS.put("CLEAR", new String[]{"0", "0", "0", "0", "0", "0", "0"});
    ...
    void testAllMethodsForVector(int vectorIndex) {
        double testValue = TEST_VECTORS[vectorIndex];

        // Przygotowanie zmiennej testowej
        ...
        // Test metody ARCTANEX
        testVar = new DoubleVariable("TEST", testValue, ctx);
        testVar.fireMethod("ARCTANEX", new DoubleVariable("", 4.0, ctx), new DoubleVariable("", 5.0, ctx));
        result = testVar.GET();
        assertEquals(parseDoubleOrSpecial(EXPECTED_RESULTS.get("ARCTANEX")[vectorIndex]), result, 0.00001,
                "ARCTANEX failed for vector " + testValue);
        ...
        result = testVar.GET();
        assertEquals(parseDoubleOrSpecial(EXPECTED_RESULTS.get("CLEAR")[vectorIndex]), result, 0.00001,
        ...
        result = testVar.GET();
        assertEquals(parseDoubleOrSpecial(EXPECTED_RESULTS.get("LENGTH")[vectorIndex]), result, 0.00001,
                "LENGTH failed for vector " + testValue);
        ...
        result = testVar.GET();
        assertEquals(parseDoubleOrSpecial(EXPECTED_RESULTS.get("MAXA")[vectorIndex]), result, 0.00001,
                "MAXA failed for vector " + testValue);
        ...
        testVar.fireMethod("MINA",
                ...
                new DoubleVariable("", 0.00001, ctx));
        result = testVar.GET();
        assertEquals(parseDoubleOrSpecial(EXPECTED_RESULTS.get("MINA")[vectorIndex]), result, 0.00001,
                "MINA failed for vector " + testValue);
