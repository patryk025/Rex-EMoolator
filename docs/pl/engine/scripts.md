...
KEYBOARD:ONKEYDOWN^DOWN={@IF("BEHNO","<","11","{BEHNO^INC();}","");@IF("BEHNO'4||BEHNO'5",{*[MATHTEST+BEHNO+"_"+TEST_SET]^RUN();},{*[MATHTEST+BEHNO]^RUN();});}
...
__INIT__:CODE={KEYBOARD^SETAUTOREPEAT(FALSE);TXTMSG^SETTEXT(["Test"+DEBUG_VALUE+"_Test_obliczen"]);TXTMSG^SHOW();TXTMSG2^SHOW();TXTMSG3^SHOW();MATHTEST1^RUN();}
```