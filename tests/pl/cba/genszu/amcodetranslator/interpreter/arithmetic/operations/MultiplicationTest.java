...
public class MultiplicationTest {
    ...
    private Variable[] expectedBoolMultiplication;
    private Variable[] expectedStringMultiplication;
    private Variable[] expectedIntegerMultiplication;
    private Variable[] expectedDoubleMultiplication;

    public MultiplicationTest(String testSetName, Context context, Variable[] expectedBoolMultiplication, Variable[] expectedStringMultiplication, Variable[] expectedIntegerMultiplication, Variable[] expectedDoubleMultiplication) {
        this.testSetName = testSetName;
        this.context = context;
        this.expectedBoolMultiplication = expectedBoolMultiplication;
        this.expectedStringMultiplication = expectedStringMultiplication;
        this.expectedIntegerMultiplication = expectedIntegerMultiplication;
        this.expectedDoubleMultiplication = expectedDoubleMultiplication;
    }
        ...
        return Arrays.asList(new Object[][] {
                {
                    ...
                    new Variable[] {
                        ...
                        new DoubleVariable("", 2.25, contexts[0]),
                        new DoubleVariable("", 1.5, contexts[0])
                    }
                },
                {
                    ...
                    new Variable[] {
                            ...
                            new IntegerVariable("", 3, contexts[1]),
                            new IntegerVariable("", 3, contexts[1]),
                            new IntegerVariable("", 0, contexts[1])
                    },
                    new Variable[] {
                            new DoubleVariable("", 0.0, contexts[1]),
                            new DoubleVariable("", 4.5, contexts[1]),
                            new DoubleVariable("", 3.75, contexts[1]),
                            new DoubleVariable("", 0.0, contexts[1])
                    }
                },
                {
                    ...
                    new Variable[] {
                            new IntegerVariable("", 3, contexts[2]),
                            new IntegerVariable("", -3, contexts[2]),
                            new IntegerVariable("", -3, contexts[2]),
                    ...
                    new Variable[]{
                            ...
                            new DoubleVariable("", -4.5, contexts[2]),
                            new DoubleVariable("", -3.75, contexts[2]),
                ...
                },
                {
                    ...
                    new Variable[] {
                            ...
                            new BoolVariable("", false, contexts[3]),
                            new BoolVariable("", true, contexts[3]),
                            new BoolVariable("", true, contexts[3])
                    },
                    new Variable[] {
                    ...
                    new Variable[] {
                            ...
                            new IntegerVariable("", -5, contexts[3]),
                            new IntegerVariable("", 1, contexts[3]),
                            new IntegerVariable("", 0, contexts[3])
                    },
                    new Variable[] {
                            new DoubleVariable("", 0.0, contexts[3]),
                            new DoubleVariable("", -32.5, contexts[3]),
                            new DoubleVariable("", 6.50007, contexts[3]),
                            new DoubleVariable("", 0.0, contexts[3])
                    }
                }
        });
    ...
    @Test
    public void testBoolMultiplication() {
        // Test BOOL + BOOL
        Variable result = executeExpression("TEST2 * TEST2_COMPARE", "BOOL");
        assertEquals(expectedBoolMultiplication[0].toString(), result.toString());

        // Test BOOL + STRING
        result = executeExpression("TEST2 * TEST_COMPARE", "BOOL");
        assertEquals(expectedBoolMultiplication[1].toString(), result.toString());

        // Test BOOL + INTEGER
        result = executeExpression("TEST2 * TEST4_COMPARE", "BOOL");
        assertEquals(expectedBoolMultiplication[2].toString(), result.toString());

        // Test BOOL + DOUBLE
        result = executeExpression("TEST2 * TEST3_COMPARE", "BOOL");
        assertEquals(expectedBoolMultiplication[3].toString(), result.toString());
    }
    ...
    public void testStringMultiplication() {
        ...
        assertEquals(expectedStringMultiplication[0].toString(), result.toString());

        // Test STRING + INTEGER
        result = executeExpression("TEST * TEST4_COMPARE", "STRING");
        assertEquals(expectedStringMultiplication[1].toString(), result.toString());

        // Test STRING + DOUBLE
        result = executeExpression("TEST * TEST3_COMPARE", "STRING");
        assertEquals(expectedStringMultiplication[2].toString(), result.toString());

        // Test STRING + BOOL
        result = executeExpression("TEST * TEST2_COMPARE", "STRING");
        assertEquals(expectedStringMultiplication[3].toString(), result.toString());
    }
    ...
    public void testIntegerMultiplication() {
        ...
        assertEquals(expectedIntegerMultiplication[0].toString(), result.toString());

        // Test INTEGER + INTEGER
        result = executeExpression("TEST4 * TEST4_COMPARE", "INT");
        assertEquals(expectedIntegerMultiplication[1].toString(), result.toString());

        // Test INTEGER + DOUBLE
        result = executeExpression("TEST4 * TEST3_COMPARE", "INT");
        assertEquals(expectedIntegerMultiplication[2].toString(), result.toString());

        // Test INTEGER + BOOL
        result = executeExpression("TEST4 * TEST2_COMPARE", "INT");
        assertEquals(expectedIntegerMultiplication[3].toString(), result.toString());
    }
    ...
    public void testDoubleMultiplication() {
        ...
        assertEquals(expectedDoubleMultiplication[0].toString(), result.toString());

        // Test DOUBLE + INTEGER
        result = executeExpression("TEST3 * TEST4_COMPARE", "DOUBLE");
        assertEquals(expectedDoubleMultiplication[1].toString(), result.toString());

        // Test DOUBLE + DOUBLE
        result = executeExpression("TEST3 * TEST3_COMPARE", "DOUBLE");
        assertEquals(expectedDoubleMultiplication[2].toString(), result.toString());

        // Test DOUBLE + BOOL
        result = executeExpression("TEST3 * TEST2_COMPARE", "DOUBLE");
        assertEquals(expectedDoubleMultiplication[3].toString(), result.toString());
    }
